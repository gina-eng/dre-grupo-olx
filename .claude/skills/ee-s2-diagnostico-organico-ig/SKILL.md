...
3. `dados/outputs/ee-s2-pesquisa-mercado.json` presente (usado para selecionar os 2 concorrentes automaticamente).
...
```bash
bash .claude/scripts/ig_organic_audit.sh dados
```

O script:
1. Lê `.credentials/meta-graph-api.json`, `dados/client.json`, `dados/outputs/ee-s2-pesquisa-mercado.json`
...
1. Salve `dados/outputs/ee-s2-diagnostico-organico-ig.json` (com `summary` no topo)
2. Atualize `dados/client.json`: progress.skills → completed, version++, history[]
3. Sugira proxima skill:
   ...
   - "Proximo passo recomendado conforme `00-playbook/07-playbook-operacional-dr-e.md`: seguir nas auditorias contratadas (ver `02-diagnostico/auditorias-contratadas.md`)."
