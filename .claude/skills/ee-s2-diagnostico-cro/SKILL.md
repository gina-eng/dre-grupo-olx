...
Verifique `dados/client.json.page_audit.fetched_at`:
- Se ausente OU mais de 7 dias → rode novo audit
- Se dentro de 7 dias → reaproveite os dados do `dados/client.json.page_audit`

**Comando:**
```bash
bash .claude/scripts/page_audit.sh dados {URL}
```
...
3. Salva raw em `dados/cache/page_audit-{timestamp}.json`
4. Condensa summary em `dados/client.json.page_audit`
...
**Comando:**
```bash
bash .claude/scripts/page_audit_deep.sh dados {URL}
```
...
6. Salva raw em `dados/cache/page_audit_deep-{timestamp}.json`
7. Condensa summary em `dados/client.json.page_audit_deep`

**Setup inicial (uma vez por ambiente):**
- `pip3 install --user -r .claude/scripts/requirements-deep.txt` (playwright, pyyaml)
...
Gere o output COMPLETO de uma vez usando os dados de `dados/client.json` (briefing, connectors) e outputs de skills dependentes em `dados/outputs/`.
...
- [ ] Usou dados reais do dados/client.json (não inventou)?
...
1. Salve em `dados/outputs/ee-s2-diagnostico-cro.json` (com campo `summary` no topo)
2. Atualize `dados/client.json`: progress.skills → completed, version++, append em history[]
