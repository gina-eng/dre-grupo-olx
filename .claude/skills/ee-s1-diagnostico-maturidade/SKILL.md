...
Leia `dados/client.json` (seção `connectors`). Se `fetched_at` existir e não for null, extraia:
...
```
O script lê `.credentials/clients.json` e salva em `dados/client.json.connectors`. Se falhar (sem workspace_id ou sem credenciais), siga com briefing apenas.
...
Leia `dados/client.json (briefing)`. Extraia:
...
Se `dados/client.json.connectors.fetched_at` não é null, incorpore no diagnóstico:
...
1. Salve em `dados/outputs/ee-s1-diagnostico-maturidade.json` (com campo `summary` no topo)
