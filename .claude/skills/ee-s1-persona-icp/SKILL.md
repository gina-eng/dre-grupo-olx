...
Se `dados/client.json` (seção `connectors`) existir e tiver `workspace.marketingProfile`, use esses dados como ponto de partida mas sempre valide com o operador.

---

## Geração

Carregue todos os dados do `dados/client.json` (seção `briefing`) e conectores.
...
- [ ] Todos os estágios têm `volume_index` (0-100, projeção [E] do cliente, Gatilho=100) e `buyer_journey.volume_is_real` está setado? (Sem isso o funil do portal fica genérico/igual em todo cliente.)
- [ ] A maior queda de `volume_index` coincide com o `critical_leakage_point`? Se `critical_leakage_point` é objeto, seu `stage` é o NOME de um estágio existente (não a frase inteira)?
...
1. Salve em `dados/outputs/ee-s1-persona-icp.json` (com campo `summary` no topo)
