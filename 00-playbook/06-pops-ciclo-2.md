...
|---|---|
| **Manter** | Práticas que funcionaram bem |
| **Ajustar** | Práticas com potencial, mas execução imperfeita |
| **Abandonar** | Ações ou abordagens que não geraram resultado |

Cada categoria vira **diretriz operacional explícita** para o Ciclo 2, respondendo:
como o sistema deve ser operado de forma diferente · quais erros não devem se repetir ·
quais práticas devem ser reforçadas.

> **Critério de qualidade:** a ausência de mudança real entre ciclos indica falha na execução deste POP.

### 2. Diagnóstico completo da nova trava · 4h · DRI: Consultor + GP
Mesmos POPs de diagnóstico do Ciclo 1, aplicados à nova restrição definida no Comitê 3 anterior.

### 3. Auditorias contratadas (quando aplicável)
### 4. Construção da nova CRT · 1h · DRI: GP + Consultor
### 5. Nuvem de Conflito · 1h (quando necessário)

### 6. Elaboração do Plano de Otimização Específico · 1h · DRI: Consultor + GP

POP exclusivo do Ciclo 2+. Transforma o diagnóstico em intervenções práticas **antes** do Comitê 1,
para que o comitê seja espaço de **validação e comprometimento**, não de construção do plano.

| Etapa | Conteúdo |
|---|---|
| Consolidação da lógica da restrição | Onde está localizada no fluxo · como se manifesta · quais causas a geram |
| Identificação das alavancas de otimização | Onde há desperdício na etapa da restrição? Onde há subutilização de capacidade? Quais ajustes aumentam conversão ou velocidade? |
| Estruturação das ações | O que será feito · onde será aplicado no fluxo · qual problema resolve · qual resultado esperado |
| Priorização preliminar | Impacto direto na restrição × facilidade de implementação × dependência de recursos |
| Métricas e critérios de sucesso | Ligadas à restrição, nunca genéricas |
| Estruturação do plano de 30 dias | Sequência lógica · DRIs · prazos realistas · dependências |
| Preparação para o Comitê 1 | Preparar o plano **e a lógica que o sustenta** |
...
POP exclusivo do Ciclo 2+. Transforma o forecast de projeção teórica em **ferramenta de decisão
baseada em evidência**, sustentando o Comitê 2.

**Métricas a revisar (previsto vs. realizado):**

| Métrica | Leitura esperada |
|---|---|
...
| Categoria | Significado |
|---|---|
| **Erro de execução** | Plano correto, execução falha |
| **Erro de hipótese** | Premissa do forecast incorreta |
| **Limite estrutural** | Sistema atingiu capacidade máxima |
...
> A pergunta central não é "o que aconteceu", é **"o que explica o comportamento do sistema"**.

### 10. Revisão do Plano de 90 Dias (PRT + TT) · 2h · DRI: Consultor + GP

POP exclusivo do Ciclo 2+. Evita a simples repetição do cronograma do Ciclo 1.

| Etapa | Conteúdo |
|---|---|
| Retomada do plano do Ciclo 1 | Como foi estruturado · quais fases foram respeitadas · onde houve desvios |
| Análise de aderência e efetividade | O tempo de diagnóstico foi suficiente? A otimização gerou o impacto esperado? Houve atraso recorrente? |
